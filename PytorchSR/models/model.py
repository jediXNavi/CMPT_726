...
from data.data_loader import TrainVoiceDataset, TestVoiceDataset, TrainVoiceDataset1, TestVoiceDataset1
...
class Model1(nn.Module):
    ...
    def data_loader(mode):
        if mode == 'train1':
            dataset = TrainVoiceDataset1()
        elif mode == 'test1':
            dataset = TestVoiceDataset1()
        else:
            raise NotImplementedError('%s mode is not implemented ! ' % mode)

        data_loader = DataLoader(dataset, batch_size=hp.train1.batch_size,
                                 shuffle=(mode == 'train1'), num_workers=hp.num_workers, drop_last=False)
        return data_loader
    ...
    @staticmethod
    def loss(pred_spec, y_spec,pred_mel,y_mel):
        loss = nn.MSELoss()
        loss_spec = loss(pred_spec,y_spec)
        loss_mel = loss(pred_mel,y_mel)
        total_loss = loss_spec + loss_mel
        return total_loss
