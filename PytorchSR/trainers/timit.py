...
from trainers.trainer1 import Trainer1, trainer_logger1
...
class TIMITTrainer1(Trainer1):
    """
    Customizing Trainer for phoneme classification
    """
    def __init__(self, model, model1, optimizer, train_dataset, test_dataset, is_cuda=True, logdir='', savedir='', topk=3):
        super().__init__(model, model1, optimizer, train_dataset, test_dataset, is_cuda=is_cuda)
        self.logdir = logdir
        self.savedir = savedir
        self.is_cuda = is_cuda
        self.topk = topk
    ...
    def train(self, epoch):
        trainer_logger1.info('------------- TRAIN Epoch : %d -------------' % epoch)
        nb_batch = len(self.train_dataset)
        self.status['train'] = defaultdict(float)

        for i, ds in enumerate(self.train_dataset):
            current_step = epoch * nb_batch + i + 1
            loss, cor, nb = TIMITModelInferencer1.train(
                epoch, i, len(self.train_dataset), self.model, self.model1, self.optimizer, ds, current_step, self.is_cuda
            )
            self.status['train']['loss'] += loss
            self.status['train']['cor'] += cor
            self.status['train']['nb'] += nb
    ...
    def test(self, epoch):
        trainer_logger1.info('------------- TEST Epoch : %d -------------' % epoch)
        nb_batch = len(self.test_dataset)
        self.status['test'] = defaultdict(float)
        self.status['test']['confusion_mat'] = np.zeros((len(PHNS), len(PHNS)), dtype=np.int32)

        for i, ds in enumerate(self.test_dataset):
            loss, cor, nb, topk_cor, topk_nb, confusion_mat = TIMITModelInferencer1.test(
                i, nb_batch, self.model, ds, is_cuda=self.is_cuda, topk=self.topk, is_confusion_mat=True
            )
            self.status['test']['loss'] += loss
            self.status['test']['cor'] += cor
    ...
    def train(epoch, batch_idx, nb_batch, network, optimizer, ds, current_step, is_cuda=True):
        ...
        if batch_idx % hp.log_step == 0:
            msg = '%d epoch / %d/%d batch / %d iteration / Elapsed: %.2f sec / total loss: %.6f / Accuracy %.6f' \
                  % (epoch, batch_idx, nb_batch, current_step, time_per_step, loss.item(), accuracy)
            trainer_logger.info(msg)
    ...
    def test(batch_idx, nb_batch, network, ds, topk=0, is_confusion_mat=False, is_cuda=True):
        ...
        if batch_idx % hp.log_step == 0:
            msg = '%d/%d batch / Elapsed: %.2f sec / Total loss: %.6f / Accuracy %.6f' \
                  % (batch_idx, nb_batch, time.time() - start_time, loss.item(), accuracy)
            if topk > 0:
...
class TIMITModelInferencer1(ModelInferencer):

    @staticmethod
    def train(epoch, batch_idx, nb_batch, network, network1, optimizer, ds, current_step, is_cuda=True):
        ...
        # setup data
        mfccs, mag_db, mel_db = list(map(lambda x: utils.to_variable(x, is_cuda), ds))

        # Forward
        logits_ppg = network(mfccs)
        ppgs, preds_ppg = module.calc_output(logits_ppg)
        pred_spec, pred_mel = network1(ppgs,mel_db,mag_db)

        loss = module1.loss(pred_spec, mag_db, pred_mel,mel_db)
        ...
        if batch_idx % hp.log_step == 0:
            msg = '%d epoch / %d/%d batch / %d iteration / Elapsed: %.2f sec / total loss: %.6f' \
                  % (epoch, batch_idx, nb_batch, current_step, time_per_step, loss.item())
            trainer_logger.info(msg)
