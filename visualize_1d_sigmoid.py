...
import sigmoid_regression as si
...
x1_ev.reshape(500, 1)
phi_train = a1.design_matrix(x1_ev, 'sigmoid', degree=0, s=2000)
y1_ev = phi_train.dot(si.w)

# x2_ev = np.linspace(np.asscalar(min(min(p.x_train), min(p.x_test))),
#                    np.asscalar(max(max(p.x_train), max(p.x_test))), num=500)
# x2_ev.reshape(500, 1)
# phi_poly_train = a1.design_matrix(x2_ev, 'polynomial', degree=3)
...
plt.plot(si.x_train, si.t_train, 'yo')
plt.plot(si.x_test, si.t_test, 'bo')
